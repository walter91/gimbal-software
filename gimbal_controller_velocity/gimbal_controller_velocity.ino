...
void loop()
{
	...
	motor1.set_gains(kp1, ki1, kd1, 'v');
	motor1.set_pins(motorPwmPin1, motorDirPin1);
	motor1.set_antiwindup(intThresh1, .025, 'v');
	motor1.set_time_filter(Ts/1000, .05, .05, 'v');
	
	motor2.set_gains(kp2, ki2, kd2, 'v');
	motor2.set_pins(motorPwmPin2, motorDirPin2);
	motor2.set_antiwindup(intThresh2, .025, 'v');
	motor2.set_time_filter(Ts/1000, .05, .05, 'v');
	...
	while(1)
	{
		...
		if( loopTime1 >= Ts)
		{
			...
			vel1 = lpf((deg1 - deg1_d1)/((float(loopTime1)/1000.0)), vel1, .05);
		...
		if( loopTime2 >= Ts)
		{
			...
			vel2 = lpf((deg2 - deg2_d1)/((float(loopTime2)/1000.0)), vel2, .05);
